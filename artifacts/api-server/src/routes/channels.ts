import { Router } from 'express';
  ...
  router.get('/payment-stickers', async (req, res) => {
    ...
    try {
      const r = await fetch(
        sbUrl(`payment_sticker_events?app_name=eq.${encodeURIComponent(app_name)}&order=created_at.asc&limit=100`),
        { headers: sbH() }
      );
      if (!r.ok) return res.status(r.status).json({ error: await r.text() });
      const events = await r.json();
  ...
  // POST /api/payment-sticker
  router.post('/payment-sticker', async (req, res) => {
    const { user_id, app_name, nombre_en_app, sticker_index } = req.body as {
      user_id?: string; app_name?: string; nombre_en_app?: string; sticker_index?: number;
    };
    if (!user_id || !app_name) return res.status(400).json({ error: 'user_id y app_name son requeridos' });
    try {
      const r = await fetch(sbUrl('payment_sticker_events'), {
        method: 'POST',
        headers: { ...sbH(), Prefer: 'return=representation' },
        body: JSON.stringify({
          user_id,
          app_name,
          nombre_en_app: nombre_en_app ?? null,
          sticker_index: typeof sticker_index === 'number' ? sticker_index : 0,
        }),
      });
      if (!r.ok) return res.status(r.status).json({ error: await r.text() });
      const [sticker] = await r.json();
      setImmediate(async () => {
        try {
          const usersR = await fetch(
            sbUrl(`channel_requests?app_name=eq.${encodeURIComponent(app_name)}&status=eq.approved&select=user_id`),
          ...
          const ids = usersRows.map((u) => u.user_id).filter(Boolean);
          if (ids.length > 0) {
            await dispatchPush(ids, `💰 Pago Recibido — ${app_name}`, `${nombre_en_app ?? 'Un usuario'} confirmó su pago`, '/canales');
          }
        } catch {}
      });
      return res.json({ ok: true, sticker });
    } catch (e: unknown) {
      return res.status(500).json({ error: e instanceof Error ? e.message : 'error' });
    }
  });
