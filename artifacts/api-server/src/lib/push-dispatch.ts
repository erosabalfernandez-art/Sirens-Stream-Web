import webPush, { PushSubscriptionJSON } from 'web-push';
    ...
    async function sendOne(
      ...
      try {
        ...
        console.error(`[push] sendNotification failed user=${user_id} status=${code} msg=${e?.message} body=${JSON.stringify(e?.body)}`);
    ...
    export async function dispatchPush(
      ...
      if (!ensureVapid() || !process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
        console.error('[push] dispatchPush: VAPID or Supabase not configured');
        return 0;
      }
      ...
      if (!subsRes.ok) {
        console.error(`[push] Supabase query failed ${subsRes.status} ${await subsRes.text()}`);
        return 0;
      }
  ...
  const payload = JSON.stringify({ title, body, url });
      ...
      const sent = results.filter(r => r.status === 'fulfilled' && (r as PromiseFulfilledResult<string>).value === 'sent').length;
      console.log(`[push] dispatchPush done: sent=${sent}/${subs.length}`);
      return sent;
    }
    ...
    export async function dispatchPushAll(
      ...
      const subsRes = await fetch(
        sbUrl('push_subscriptions?select=user_id,subscription'),
        { headers: sbHeaders() }
      );
      ...
      const sent = results.filter(r => r.status === 'fulfilled' && (r as PromiseFulfilledResult<string>).value === 'sent').length;
      console.log(`[push] dispatchPushAll done: sent=${sent}/${subs.length}`);
      return sent;
    }
