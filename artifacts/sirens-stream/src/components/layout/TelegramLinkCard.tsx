...
import { Send, CheckCircle2, Link2Off, Loader2, ExternalLink, RefreshCw, AlertTriangle } from 'lucide-react'
...
export function TelegramLinkCard({ userId, lang = 'es' }: Props) {
    ...
    const iv = setInterval(async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (data.linked) { setStatus(data); setWaitingLink(false); setVerifyResult(null) }
        }
      } catch { /* ignore */ }
    }, 3000)
  ...
  async function handleVerify() {
    ...
    setVerifyError(null)
    try {
      const res = await fetch(`${API}/api/telegram/verify/${userId}`, { method: 'POST' })
      ...
      if (data.ok) {
        ...
        setVerifyError(data.error ?? 'unknown')
      }
    } catch {
      setVerifyResult('error')
  ...
  return (
    <div className="bg-[#0d0d1e] border border-purple-500/10 rounded-2xl p-5 mb-4">
            ...
            <div className="flex flex-col items-end gap-1.5">
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  title={pt ? 'Verificar conexão' : 'Verificar conexión'}
                  className="flex items-center gap-1.5 bg-white/5 hover:bg-blue-500/10 text-white/40 hover:text-blue-400 text-xs font-semibold px-3 py-1.5 rounded-lg transition-all border border-white/10 hover:border-blue-500/20 disabled:opacity-40">
                  {verifying
                    ? <Loader2 className="w-3 h-3 animate-spin" />
                    : <RefreshCw className="w-3 h-3" />}
                  {verifying ? (pt ? 'Verificando...' : 'Verificando...') : (pt ? 'Verificar' : 'Verificar')}
                </button>
                <button
      ...
      {verifyResult === 'ok' && (
        <div className="mt-3 pt-3 border-t border-white/5 flex items-center gap-2 text-green-400 text-xs font-semibold">
          <CheckCircle2 className="w-3.5 h-3.5 shrink-0" />
          {pt ? '✓ Conexão funcionando! Verifique seu Telegram — enviamos uma mensagem de teste.' : '✓ ¡Conexión funcionando! Revisa tu Telegram — te enviamos un mensaje de prueba.'}
        </div>
      )}

      {verifyResult === 'error' && (
        <div className="mt-3 pt-3 border-t border-white/5 space-y-1.5">
          <div className="flex items-start gap-2 text-amber-400 text-xs font-semibold">
            <AlertTriangle className="w-3.5 h-3.5 shrink-0 mt-0.5" />
            <span>{pt ? '⚠ Erro de conexão com o Telegram.' : '⚠ Error de conexión con Telegram.'}</span>
          </div>
          <p className="text-white/35 text-xs leading-relaxed pl-5">
            {pt
              ? 'O bot não conseguiu te alcançar. Peça a um administrador que elimine sua vinculação para poder reconectar do zero.'
              : 'El bot no pudo alcanzarte. Pídele a un administrador que elimine tu vinculación para que puedas reconectar desde cero.'}
          </p>
        </div>
      )}
