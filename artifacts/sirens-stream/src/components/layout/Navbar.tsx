import { Link, useLocation } from "wouter";
  ...
  import {User, LogOut, Shield, FileSpreadsheet, DollarSign, Radio, Globe, CheckCircle} from "lucide-react";
  ...
  function CierreModal({ onClose }: { onClose: () => void }) {
    ...
    return (
      <div className="fixed inset-0 z-[200] flex items-start justify-center pt-20 px-4" onClick={onClose}>
        ...
        <div className="relative bg-[#0d0d1e] border border-white/10 rounded-2xl p-6 max-w-md w-full shadow-2xl" onClick={e => e.stopPropagation()}>
          <p className="text-xs font-bold uppercase tracking-widest text-white/30 mb-4">Cierre de Semana</p>
          ...
          {loading ? (
            ...
            <div className="text-center py-4">
              ...
              <p className="text-white/50 text-sm mt-1">{result.message ?? 'Todas las personas han confirmado su pago. Las nóminas han sido limpiadas.'}</p>
            </div>
          ) : result && !result.allConfirmed && result.pending ? (
            <div>
              <p className="text-amber-400 font-bold mb-1">⚠️ Pendiente de confirmar</p>
              <p className="text-white/40 text-xs mb-3">Estas personas aún no han confirmado recibir su pago:</p>
              <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                {result.pending.map((p, i) => (
                  <div key={i} className="flex items-center justify-between bg-[#1a1a2e] rounded-xl px-4 py-2.5">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full border ${p.type === 'agente' ? 'bg-amber-500/15 text-amber-300 border-amber-500/20' : 'bg-purple-500/15 text-purple-300 border-purple-500/20'}`}>
                      {p.type}
                    </span>
                  </div>
                ))}
